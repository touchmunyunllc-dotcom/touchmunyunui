...
type ProductEditTab = 'basic' | 'pricing' | 'media' | 'storefront' | 'variants';

const PRODUCT_EDIT_TABS: { id: ProductEditTab; label: string }[] = [
...
export default function EditProduct() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      notificationService.error(error.response?.data?.message || 'Failed to update product');
    } finally {
      setSaving(false);
    }
  };
  ...
  const currentTabIndex = PRODUCT_EDIT_TABS.findIndex((tab) => tab.id === activeTab);
  const isFirstTab = currentTabIndex === 0;
  const isLastTab = currentTabIndex === PRODUCT_EDIT_TABS.length - 1;

  const goToPreviousTab = () => {
    if (!isFirstTab) setActiveTab(PRODUCT_EDIT_TABS[currentTabIndex - 1].id);
  };

  const goToNextTab = () => {
    if (!isLastTab) setActiveTab(PRODUCT_EDIT_TABS[currentTabIndex + 1].id);
  };
    ...
    <Layout>
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-12 bg-primary min-h-screen">
        ...
        <form onSubmit={handleSubmit} className="bg-primary/80 backdrop-blur-xl rounded-3xl shadow-glass-lg p-8 border-2 border-foreground/10">
          <div className="mb-6">
            <h2 className="text-xl font-bold text-foreground">{PRODUCT_EDIT_TABS[currentTabIndex].label}</h2>
            <p className="text-sm text-foreground/60 mt-1">
              Step {currentTabIndex + 1} of {PRODUCT_EDIT_TABS.length}
            </p>
          </div>
